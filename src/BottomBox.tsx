...
import {View, StyleSheet, TouchableWithoutFeedback, ViewStyle, StyleProp, BackHandler, Platform} from 'react-native';
...
type Props = {
    forwardRef?: (ref: {open: () => void, close: () => void}) => void,
    children: any,
    height: number,
...
function BottomBox(props: Props){
    const {forwardRef, children, height, containerStyle, transition: _transition} = props;
    const insets = useSafeAreaInsets();
    const [visible, setVisible] = useState(false);
    const goUp: Animated.Value<0 | 1> = useMemo(() => new Value(0), []);
    const goDown: Animated.Value<0 | 1> = useMemo(() => new Value(0), []);
    ...
    return (
        <View style={[StyleSheet.absoluteFill, {zIndex: visible ? 100 : -100, display: visible ? 'flex' : 'none'}]}>
            <TouchableWithoutFeedback onPress={() => {
                ...
                <Animated.View
                    style={[StyleSheet.absoluteFill, {backgroundColor: 'black', opacity: interpolate(
                        transition,
                        {
                            inputRange: [0, 1],
                            outputRange: [0, 0.4]
